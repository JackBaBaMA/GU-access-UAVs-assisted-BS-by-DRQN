...
# DRQN model and associated relay buffer model 
...
class DRQN(nn.Module):
    ...
    def forward(self, x):
        x, _ = self.lstm(x)
        x = self.fc(x[:, -1, :])  # Use only the last LSTM output
        return x
...
# Hyperparameters
input_dim = int(((network_area[0] / moving_step)**2)) #num_states
hidden_dim = 128
output_dim = num_actions  # num_action
learning_rate = 0.001
gamma = 0.99
epsilon = 0.1
batch_size = 32
...
UAV_learning_0 = DRQN(input_dim, hidden_dim, output_dim) 
UAV_learning_1 = DRQN(input_dim, hidden_dim, output_dim) 
UAV_learning_2 = DRQN(input_dim, hidden_dim, output_dim) 
